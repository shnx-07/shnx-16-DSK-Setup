...
class BackendApplication:
    ...
    async def run(self) -> None:
        ...
        try:
            await self._wallpaper.restore_saved()

        except Exception:
            self._logger.exception(
                "Unexpected wallpaper restore failure"
            )
    ...
    async def _handle_message(
        ...
        if message.command == "screenshot.full":
            return await self._handle_screenshot_full(
                message
            )

        if message.command == "screenshot.region":
            return await self._handle_screenshot_region(
                message
            ) 
        if message.command == "screen.start":
            return await self._handle_screen_start(message)

        if message.command == "screen.pause":
            return await self._handle_screen_pause(message)

        if message.command == "screen.resume":
            return await self._handle_screen_resume(message)

        if message.command == "screen.stop":
            return await self._handle_screen_stop(message)

        if message.command == "screen.status":
            return await self._handle_screen_status(message)
    ...
    async def _broadcast_screen_state(
        self,
        state: dict,
    ) -> None:
        await self._server.broadcast(
            event_message(
                event="screen.recordingState",
                payload=state,
            )
        )

    async def _handle_screen_start(
        self,
        message: IncomingMessage,
    ) -> dict:
        ...
        if (
            ...
            return error_message(
                request_id=message.request_id,
                code="invalid_output",
                message="Monitor output is required",
            )
    ...
    async def _handle_screen_pause(
        self,
        message: IncomingMessage,
    ) -> dict:
        try:
    ...
    async def _handle_screen_resume(
        self,
        message: IncomingMessage,
    ) -> dict:
        try:
    ...
    async def _handle_screen_status(
        self,
        message: IncomingMessage,
    ) -> dict:
        result = self._recorder.status()
    ...
    async def _handle_screenshot_full(
        self,
        message: IncomingMessage,
    ) -> dict:
        output = message.payload.get(
        ...
        if output is not None and not isinstance(output, str):
            return error_message(
                request_id=message.request_id,
                code="invalid_screenshot_output",
                message=(
                    "screenshot.full output must be "
                    "a string or null."
                ),
            )
        ...
        try:
            ...
            self._logger.exception(
                "Unexpected full screenshot failure"
            )
            ...
            return error_message(
                ...
                code="screenshot_failed",
                message=(
                    "Full screenshot failed unexpectedly."
                ),
            )
    ...
    async def _handle_screenshot_region(
        self,
        message: IncomingMessage,
    ) -> dict:
        geometry = message.payload.get(
            "geometry",
            "",
        )
        ...
        if not isinstance(geometry, str):
            return error_message(
                request_id=message.request_id,
                code="invalid_screenshot_geometry",
                message=(
                    "screenshot.region geometry must "
                    "be a string."
                ),
            )
        ...
        if not geometry:
            return error_message(
                ...
                code="invalid_screenshot_geometry",
                message=(
                    "screenshot.region requires geometry."
                ),
            )

        try:
            result = await asyncio.to_thread(
                self._screenshot.capture_region,
                geometry,
            )
            ...
            self._logger.exception(
                "Unexpected region screenshot failure"
            )
            ...
            return error_message(
                ...
                code="screenshot_failed",
                message=(
                    "Region screenshot failed unexpectedly."
                ),
            )
