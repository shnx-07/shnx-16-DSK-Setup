...
class WallpaperService:
    ...
    def _save_state(
        ...
        try:
            ...
            self._logger.info(
                "Saved wallpaper state: %s",
                wallpaper,
            )

        except OSError as error:
            self._logger.error(
                "Could not save wallpaper state: %s",
                error,
            )
    ...
    def _load_state(
        ...
        try:
            ...
            self._logger.warning(
                "Could not read wallpaper state: %s",
                error,
            )
        ...
        wallpaper = Path(
            os.path.expandvars(
                os.path.expanduser(path_raw)
            )
        ).resolve()

        if not wallpaper.is_file():
            self._logger.warning(
                "Saved wallpaper no longer exists: %s",
                wallpaper,
            )
        ...
        if detected_type is None:
            self._logger.warning(
                "Saved wallpaper type is unsupported: %s",
                wallpaper,
            )

            return None

        # Trust detection rather than stale JSON.
        return wallpaper, detected_type
    ...
    async def restore_saved(
        ...
        if saved is None:
            self._logger.info(
                "No saved wallpaper to restore"
            )

            return False

        wallpaper, media_type = saved

        try:
            self._logger.info(
                "Restoring saved wallpaper: %s",
                wallpaper,
            )
            ...
            self._logger.error(
                "Could not restore saved wallpaper %s: %s",
                wallpaper,
                error,
            )
        ...
        self._logger.info(
            "Saved wallpaper restored successfully"
        )
